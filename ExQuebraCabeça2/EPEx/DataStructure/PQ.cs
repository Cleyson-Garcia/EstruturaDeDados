...
namespace ProjetoGrafos.DataStructure
{
    class PQ
    {
        ...
        {
            int indice = size - 1;
            while(!IsRaiz(indice) && _elementos[indice].GetF() < GetPai(indice).GetF())
            {
        ...
        private void HeapDown()
        {
            ...
            while(TemFilhoEsquerdo(indice))
            {
                ...
                if(TemFilhoDireito(indice) && GetFilhoDireita(indice).GetF()< GetFilhoEsquerda(indice).GetF())
                {
                    menorIndice = IndexFilhoDireita(indice);
                }

                if(_elementos[menorIndice].GetF() >= _elementos[indice].GetF())
                {
                    break;
                }
